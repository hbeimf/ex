...
process_channel_frame(Frame, Channel, ChPid, AState) ->
    case rabbit_command_assembler:process(Frame, AState) of
        {ok, NewAState}                  -> 
                ?LOG(#{frame => Frame, channel => Channel, ch_pid => ChPid}),
                NewAState;
        {ok, Method, NewAState}          -> 
                            ?LOG(#{frame => Frame, channel => Channel, ch_pid => ChPid, method => Method}),
                            rabbit_channel:do(ChPid, Method),
                                            NewAState;
        {ok, Method, Content, NewAState} -> 
            ?LOG(#{frame => Frame, channel => Channel, ch_pid => ChPid, method => Method, content => Content}),
            %% 这一句将数据 转给业务 回调
            rabbit_channel:do(ChPid, Method,Content),
            NewAState;
        {error, Reason}                  -> 
            ?LOG(#{frame => Frame, channel => Channel, ch_pid => ChPid}),
            ChPid ! {channel_exit, Channel,
                                                     Reason},
                                            AState
    end.
...
handle_call({open_channel, ProposedNumber, Consumer, InfraArgs}, _,
    ...
    ?LOG(#{open_channel => {ProposedNumber, Consumer, InfraArgs}}),
...
handle_cast({pass_frame, ChNumber, Frame}, State) ->
    ?LOG(#{ch_number => ChNumber, frame=> Frame, state => State}),
...
handle_open_channel(ProposedNumber, Consumer, InfraArgs,
    ...
    case new_number(ProposedNumber, State) of
        {ok, Number} ->
            ...
            ?LOG(#{ch => {Ch, internal_register}, number => Number}),
...
internal_pass_frame(Number, Frame, State) ->
    ?LOG(#{number=> Number, frame => Frame,  state => State}),
    %% 报错就是因为这里返回undefined, 所以这个actor的初始化得搞清楚 ，
    %% 为嘛会导致 channel_pid 找不到， 并没有判断 是否活着，只是简单的查找channel_pid
    % map_num_pa
