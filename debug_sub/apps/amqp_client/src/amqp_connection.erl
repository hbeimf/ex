...
start(AmqpParams) ->
    ...
    ?LOG(#{connection_pid => Connection, amqp_params =>  AmqpParams1, pos => 1}),
...
open_channel(ConnectionPid, ChannelNumber,
    ...
    % ?LOG(#{open_channel => {ConnectionPid, ChannelNumber, Consumer}, step => 1}),
     ?LOG(#{connection_pid => ConnectionPid, channel_number => ChannelNumber, consumer =>  Consumer, step => 1}),
