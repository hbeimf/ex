...
start_link(TypeSup, AMQPParams) ->
    %% 此处启动连接 actor
    ?LOG(#{type_sup =>TypeSup , amqp_params =>  AMQPParams}),
    gen_server:start_link(?MODULE, {TypeSup, AMQPParams}, []).

connect(Pid) ->
    ?LOG(#{connection_pid => Pid, pos => 2}),
    gen_server:call(Pid, connect, infinity).

open_channel(Pid, ProposedNumber, Consumer) ->
    % ?LOG(#{open_channel => {ConnectionPid, ChannelNumber, Consumer}, step => 2}),
    ?LOG(#{connection_pid => Pid, channel_number => ProposedNumber, consumer =>  Consumer, step => 2}),

    case gen_server:call(Pid,
                         {command, {open_channel, ProposedNumber, Consumer}},
                         infinity) of
        {ok, ChannelPid} -> ok = amqp_channel:open(ChannelPid),
                            ?LOG(#{connection_pid => Pid, channel_number => ProposedNumber, consumer =>  Consumer,  channel_pid => ChannelPid, step => 3}),
                            {ok, ChannelPid};
        Error            -> 
                            ?LOG(#{connection_pid => Pid, channel_number => ProposedNumber, consumer =>  Consumer,  error => Error, step => 4}),
                            Error
    end.
...
handle_call(connect, _From, {TypeSup, AMQPParams}) ->
    ...
    case Mod:connect(AMQPParams, SIF, TypeSup, MState) of
        {ok, Params} ->
            %% 此处建立 连接 
            ?LOG(#{connect_pid => self(), mod => Mod, connect_params => {AMQPParams, SIF, TypeSup, MState}}),
            % mod => amqp_network_connection}
...
handle_command({open_channel, ProposedNumber, Consumer}, _From,
    ...
    ?LOG(#{open_channel => {ProposedNumber, Consumer}, mod => Mod}),
